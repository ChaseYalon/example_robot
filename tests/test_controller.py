...
def test_shot_controller_controller():
    ...
    assert c.shooter.velocity == ctrl.volts_to_velocity(
        0.8 * c.MAX_VOLTS.in_unit(volts)
    )
...
def test_shot_controller_calc_optimal_volts():
    ...
    landing = ctrl.vertical_position_at_horizontal_distance(
        velocity, ctrl.SHOOTER_ANGLE, 3
    )
