...
# Not sure I like this, the alternative is a setup.py, a conftest.py, or a few others
...
class MyRobot(wpilib.TimedRobot):
    ...
    def teleopPeriodic(self):
        ...
        # <Swerve>
        self.swerve.drive(
            meters_per_second.of(
                self.primary.getLeftX()
                * self.MAX_LINEAR_VELOCITY.in_units(meters_per_second)
            ),
            meters_per_second.of(
                self.primary.getLeftY()
                * self.MAX_LINEAR_VELOCITY.in_units(meters_per_second)
            ),
            radians_per_second.of(
                self.primary.getRightX()
                * self.MAX_ANGULAR_VELOCITY.in_units(radians_per_second)
            ),
    ...
    def autonomousInit(self):
        self.ctx = AutoContext(
            self.swerve, self.intake, self.shot_controller, self.odometry
        )
