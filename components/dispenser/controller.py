...
def vertical_distance_over_time(
    initialVelocity: float, angle: float, time: float
) -> float:
    return (
        BALL_EXIT_HEIGHT
...
def time_to_reach_target_horizontal_distance(
    initialVelocity: float, angle: float, horizontalDistance: float
) -> float:
    return horizontalDistance / (initialVelocity * math.cos(math.radians(angle)))


def vertical_position_at_horizontal_distance(
    initialVelocity: float, angle: float, horizontalDistance: float
) -> float:
    time = time_to_reach_target_horizontal_distance(
        initialVelocity, angle, horizontalDistance
    )
...
def hits_backboard(velocity: float, horizontalDistance: float) -> bool:
    ...
    y_back = vertical_position_at_horizontal_distance(
        velocity, SHOOTER_ANGLE, back_edge_x
    )
...
def shot_is_possible(horizontalDistance: float) -> bool:
    ...
    while volts <= 12.0:
        ...
        expectedVerticalPos = vertical_position_at_horizontal_distance(
            velocity, SHOOTER_ANGLE, horizontalDistance
        )

        if HOOP_HEIGHT <= expectedVerticalPos <= NET_HEIGHT and not hits_backboard(
...
def calc_optimal_volts(horizontalDistance: float) -> float:
    ...
    while True:
        ...
        elif expectedVerticalPos > NET_HEIGHT or hits_backboard(
            velocity, horizontalDistance
        ):
            maxVolts = (
                workingVolts  # long, or clangs off the backboard - needs less power
            )
        else:
            break  # clean make
